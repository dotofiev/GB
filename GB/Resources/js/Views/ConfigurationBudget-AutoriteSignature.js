        ...
        $.ajax({
            ...
            success: function (resultat) {
                ...
                if (!resultat.notification.est_echec) {
                    ...
                    $('#form_montant_signature').val(resultat.notification.donnee.montant_signature);
                    $('#form_limite_decouvert_client').val(resultat.notification.donnee.limite_decouvert_client);
                    $('#form_debit_max_client').val(resultat.notification.donnee.debit_max_client);
                    $('#form_credit_max_client').val(resultat.notification.donnee.credit_max_client);
                    $('#form_montant_max_ligne_credit').val(resultat.notification.donnee.montant_max_ligne_credit);
                    $('#form_montant_limite_pret').val(resultat.notification.donnee.montant_limite_pret);
...
$(
    function () {
            ...
            form.on("submit",
                function (e) {
                    ...
                    if (form.parsley().isValid()) {
                        ...
                        // -- SUppression des espaces sur les montant -- //
                        $('.gb-temp-mask-nombre').each(function () {
                            // -- Suppression des espaces -- //
                            gbConsole('Avant: ' + $(this).val());
                            $(this).val(
                                $(this).val().gbReplace(' ', '')
                            );
                            gbConsole('Après: ' + $(this).val());
                        });
        ...
        try {
            ...
            btn_supprimer.on("click",
                function () {
                    ...
                    var selection = $('input[name="autoriteSignature"]:checked');
                    ...
                    for (var i = 0; i < selection.length; i++) {
                        ids.push(selection[i].replace('autoriteSignature=autoriteSignature_', ''));
                    }
        ...
        // -- Action d'application du mask sur les elements nombre -- //
        try {

            // -- Mise à jour de l'action de suppression -- //
            $('.gb-temp-mask-nombre').mask('# ##0', { reverse: true });
